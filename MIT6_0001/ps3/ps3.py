...
def get_word_score(word, n):
    ...
    for char in word:
        word_points += SCRABBLE_LETTER_VALUES[char.lower()]
...
def update_hand(hand, word):
    ...
    for char in word.lower():
        ...
        elif char in hand and hand[char]>0:
            new_hand[char] -= 1

        else:
            new_hand[char] = 0 

    new_hand = strip_empty_keys(new_hand)

    return new_hand


def is_in_wordlist(word, word_list):
    """
    Returns True if word is in the word_list or a word from the word_list 
    can be formed if wildcard is present. Otherwise, returns False.
    ...
    if '*' in word:
        for char in VOWELS:
                new_word = word[:word.find('*')] + char + word[word.find('*')+1:]
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' in word and not is_in_wordlist(word, word_list):
        return False

    word_freq = get_frequency_dict(word.lower())

    for char in word_freq:
        if char not in hand or word_freq[char] > hand[char]:
            return False
    return True


def strip_empty_keys(hand):
    '''
    Returns hand without keys whose value is 0.
...
def substitute_hand(hand, letter):
    ...
    for char in VOWELS and CONSONANTS:
        if char not in hand:
            all_letters += char
    ...
    for char in hand:
        if char == letter:
            ...
            # create new key with new letter
            new_hand[x] = new_hand.get(char)
            # delete old key
            del new_hand[char]
    return new_hand
